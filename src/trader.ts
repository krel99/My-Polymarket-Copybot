...
export class TradeExecutor {
  ...
  /**
   * Compares the current best market price against the price at which the tracked
   * wallet originally traded. If the market has moved against us by more than
   * `maxPriceDrift`, the trade is rejected before we even apply execution slippage.
   *
   * For BUY:  a higher current ask means the market moved against us.
   * For SELL: a lower current bid means the market moved against us.
   */
  private checkPriceDrift(originalPrice: number, currentBestPrice: number, side: "BUY" | "SELL"): void {
    const { maxPriceDrift } = config.trading;
    if (maxPriceDrift <= 0) return; // 0 disables the check

    const drift = side === "BUY" ? (currentBestPrice - originalPrice) / originalPrice : (originalPrice - currentBestPrice) / originalPrice;

    if (drift <= 0) {
      console.log(`   Price drift: ${(drift * 100).toFixed(2)}% (price improved since original trade)`);
      return;
    }

    console.log(
      `   Price drift: +${(drift * 100).toFixed(2)}% ` +
        `(original: ${originalPrice.toFixed(4)}, current: ${currentBestPrice.toFixed(4)}, ` +
        `limit: ${(maxPriceDrift * 100).toFixed(2)}%)`,
    );

    if (drift > maxPriceDrift) {
      throw new Error(
        `Price drifted ${(drift * 100).toFixed(2)}% from the original trade price ` +
          `(max allowed: ${(maxPriceDrift * 100).toFixed(2)}%). ` +
          `Original: ${originalPrice.toFixed(4)}, current best: ${currentBestPrice.toFixed(4)}`,
      );
    }
  }

  async executeCopyTrade(originalTrade: Trade, copyNotionalOverride?: number): Promise<CopyExecutionResult> {
  ...
  private async executeWithRetry<T>(fn: () => Promise<T>, attempt: number = 1): Promise<T> {
    try {
      ...
      const delay = Math.min(this.RETRY_CONFIG.initialDelay * Math.pow(this.RETRY_CONFIG.backoffMultiplier, attempt - 1), this.RETRY_CONFIG.maxDelay);
  ...
  private isRetryableError(error: any): boolean {
    ...
    if (errorMsg.includes("price drifted")) {
      console.log("   ⚠️  Pre-flight check rejected trade — not retrying");
      return false;
    }
  ...
  private async executeLimitOrder(originalTrade: Trade, copyNotional: number): Promise<CopyExecutionResult> {
    ...
    this.checkPriceDrift(originalTrade.price, bestPrice, originalTrade.side);
